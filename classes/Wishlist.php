...
class Wishlist extends ObjectModel
{
    ...
    public static function getByIdCustomer($idCustomer)
    {
        ...
        $cacheId = __CLASS__.'_'.(int) $idCustomer.'-'.(int) Shop::getContextShopID().'-'.(int) Shop::getContextShopGroupID();
        if (!Cache::isStored($cacheId)) {
    ...
    {
        if (!Validate::isUnsignedId($idWishlist) OR
            !Validate::isUnsignedId($idCustomer) OR
            !Validate::isUnsignedId($idProduct) OR
        ...
        if (empty($result) === false AND sizeof($result)) {
            if (($result['quantity'] + $quantity) <= 0) {
                return (Wishlist::removeProduct($idWishlist, $idCustomer, $idProduct, $idProductAttribute));
            } else {
                return (Db::getInstance()->execute('
