...
class WishListId
{
    ...
    {
        $this->assertIntegerIsGreaterThanZero($wishListId);
    ...
    private function assertIntegerIsGreaterThanZero($value)
    {
        if (!is_int($value) || 0 >= $value) {
            throw new WishListConstraintException(
                sprintf(
                    'WishList id must be integer greater than zero, but %s given.',
                    var_export($value, true)
                ),
                WishListConstraintException::INVALID_WISH_LIST_ID
            );
        }
    }
}
