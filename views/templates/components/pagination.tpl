...
<nav class="wishlist-pagination pagination">
  <template v-if="display">
    <div class="col-md-4">
      {l s='Showing' mod='Modules.Blockwishlist.Shop'} ((minShown)) - ((maxShown)) {l s='de' mod='blockwishlist'} ((total)) {l s='item(s)' mod='blockwishlist'}
    </div>
        ...
        <li :class="{literal}{current: page.current}{/literal}" v-for="page of pages">
          <a class="js-search-link" @click="paginate(page)" key="page.page" :class="{literal}{disabled: page.current, next: page.type === 'next', previous: page.type === 'previous'}{/literal}">
            <span v-if="page.type === 'previous'">
              <i class="material-icons">keyboard_arrow_left</i> {l s='Previous' mod='Modules.Blockwishlist.Shop'} 
            </span>
            ...
            <span v-if="page.type === 'next'">
              {l s='Next' mod='Modules.Blockwishlist.Shop'} <i class="material-icons"></i>
            </span>
          </a>
        </li>
      </ul>
